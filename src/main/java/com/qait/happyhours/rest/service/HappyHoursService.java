...
import com.qait.happyhours.dto.DealDTO;
...
public class HappyHoursService {
	...
	public String text(@HeaderParam("Accept") String s) {
		System.out.println(s);
		DealDTO deal=new DealDTO();
		deal.setLatitude(28.585148);
		deal.setLongitude(77.311413);
		deal.setDistance((double) 100);
		searchDealByGeoLocation(s, deal);
		return "Its working";
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response searchDeal(@HeaderParam("Authorization") String authorizationValue, @QueryParam("dealString") String searchStr) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response searchDealByGeoLocation(@HeaderParam("Authorization") String authorizationValue, DealDTO dealDTO) {
		...
		List<Deal> allDealList = dealService.getAllDealsList();
		...
		for (Deal deal2 : allDealList) {
			...
			if(distance < dealDTO.getDistance()){
				dealList.add(deal2);
			}
		}
